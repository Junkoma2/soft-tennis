...
const Y_RANGE_BACK  = { min: 1.0, max: 14.0 };
const Y_RANGE_FRONT = { min: 0.6, max: 14.0 };
  ...
  if (!receiveDone && state === "rally" && ball.lastHitter === opponentTeam) {
    if (p === receiverPlayerFor(myTeam)) {
      ...
      if (ball.bounces >= 1) {
        ...
        ty = homeSign > 0 ? Math.min(COURT.halfL + 2.2, Math.max(4.0, ball.y + ball.vy * 0.2))
                          : Math.max(-(COURT.halfL + 2.2), Math.min(-4.0, ball.y + ball.vy * 0.2));
  ...
  if (p === myFront) {
    ...
    if (state === "rally" && ball.lastHitter === opponentTeam) {
      ...
      if (ball.bounces >= 1) {
        ...
        tx = ball.x + ball.vx * 0.2;
        ty = homeSign > 0
          ? Math.min(COURT.halfL + 2.2, Math.max(4.5, ball.y + ball.vy * 0.2))
          : Math.max(-(COURT.halfL + 2.2), Math.min(-4.5, ball.y + ball.vy * 0.2));
      } else if (landing && landing.y * homeSign > 0 && insideCourt(landing.x, landing.y)) {
    ...
    myBack.x = Math.max(-6.3, Math.min(6.3, myBack.x));
  }
}

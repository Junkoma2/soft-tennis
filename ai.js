...
export function tryReturnAI(side) {
  ...
  // ---- 後衛の返球（ノーバウンド/ライジング/降下を実行） ----
  // 位置取り(updateContactLatchの選択打点)へ詰めた後衛が、自陣側で打てる高さに来た球を打つ。
  //   ・ノーバウンド: ネットを越えて落下中(bounces=0 かつ vz<0)の球を捉える（上昇中・相手側は対象外）。
  //   ・バウンド後: 上昇(ライジング)・頂点・降下のいずれでも打てる（速い深い球は頂点付近で捉える）。
  //   z は 0.3〜2.3m を打てる高さとする。どこで打つかはラッチした立ち位置＋リーチが決める。
  //   サーブ(receiveDone前)はこの経路では打たない（レシーブは上の専用ブロックが担当）。
  const backHittable = receiveDone && ball.z > 0.3 && ball.z < 2.3 &&
    ball.y * homeSign > 0 && (ball.bounces >= 1 || ball.vz < 0);
  if (backHittable) {

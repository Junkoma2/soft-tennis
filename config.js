...
export const TUNING = {
  ...
  pos: {
    ...
    // ── 前衛（ネット前選手）のセンター寄せ ──
    // 相手が打つ瞬間、守備範囲は変えずに範囲内でセンター側境界（中心線）へ寄って立つ。
    frontCenterHug: 0.75,   // 寄せの最大率（0=ゾーン中央のまま, 1=中心線ぴったり）
    frontHugNearDist: 5.0,  // 相手打点との距離がこれ以下なら寄せ0（反応が間に合わないため）(m)
    frontHugFarDist: 9.0,   // この距離以上で最大寄せ。間は線形補間(m)
  ...
  tempo: {
    ...
    // 2026-09: 0.42→0.30。打撃入力からボールが実際に飛び始めるまで（打球イベントで
    // ボールを保持し、impactPhase到達フレームで発射する既存の同期方式はそのまま）の
    // 体感ラグが大きすぎるとの指摘を受けて短縮。impactPhase(比率)は変えず全体を
    // 均等に速めるだけなので、テイクバック→インパクト→フォロースルーの見た目の
    // 配分比は変わらない（キーフレーム位相はanimation3d.jsのSWING_KEYS参照）。
    swingDuration: 0.30, // インパクトから振り抜きが収まるまでの表示時間(秒)
  ...
  // ボレー（ノーバウンド返球）の調整
  volley: {
    returnLockout: 0.3, // 打たれた瞬間からこの秒数は誰も返球できない(秒)。前衛同士のあり得ない即ボレー連鎖を防ぐ
    speedMul: 0.85,     // ボレーの球速係数。体勢を作れないぶん球威を落とす（スマッシュは対象外）
  },
  // 移動の速さ（m/s）
  move: {
    // 2026-07: 「足が遅すぎる」というフィードバックを受け、体感速度に直結する値を見直し。
    // 特に sidestepMaxSpeed はラリー中の短い切り返しでほぼ常に効いてくる実効速度で、
    // playerSpeed（最高速）よりも体感に与える影響が大きいため合わせて引き上げる。
    playerSpeed: 8.5,   // 操作キャラの足の速さ
    partnerSpeed: 4.2,  // 味方AIの足の速さ（未使用: aiSpeedへ統一済み。旧値のまま残置）
    cpuBackSpeed: 3.8,  // 相手後衛の足の速さ（未使用: aiSpeedへ統一済み。旧値のまま残置）
    cpuFrontSpeed: 3.6, // 相手前衛の足の速さ（未使用: aiSpeedへ統一済み。旧値のまま残置）
    aiSpeed: 4.8,        // moveAutoAI 共通の基本速度（統一AI・観戦モード対応）。
                          // 従来はplayerSpeedの半分程度で味方/相手の動きだけ遅く見えていたため引き上げ
    ...
    // サイドステップ域（横速度が sidestepMaxSpeed 以下）専用の加減速。
    // 見た目のサイドステップに合わせ、横方向は動き出しと止まりだけ通常走行より
    // キビキビ反応させる（サイドステップは重心移動が小さく、初速の立ち上がり・
    // 減速が速いという実際の動きに寄せる）。中〜高速域は通常の accel/decel のまま。
    sidestepAccel: 85.0, // 横方向・低速域の加速（m/s^2）
    sidestepDecel: 110.0, // 横方向・低速域の減速（m/s^2）
    // 見た目の移動表現（3D描画のみ・当たり判定/移動速度そのものには影響しない）:
    // 近距離の横移動はサイドステップ、遠距離は体を横向きにして走る、の2種を
    // 実際の移動速度で切り替える。しきい値を境に別のアニメ・ターン処理を使う。
    // sidestepAccel/Decel の適用域としても共有する（見た目と物理の切り替え点を揃える）。
    sidestepMaxSpeed: 4.2, // これ以下の横移動速度まではサイドステップ（正面/ボール向きを保つ）
    runTurnSpeed: 9.0,     // /秒。走り出す遠距離移動で進行方向へ体を向け直す速さ
    // 横移動の動き出しから約0.7秒はサイドステップ局面として扱う（時間ベース）。
    // この間は横方向の目標速度をsidestepMaxSpeedでクランプし、sidestepAccel/Decelで
    // 加減速する。経過後は通常の最高速まで加速する「走り」へ移行する。
    // 横入力が無くなった/反転したときはこのフェーズタイマーをリセットし、
    // 次に動き出す際は再びサイドステップから始まる。
    sidestepPhaseSec: 0.7, // サイドステップ局面が持続する時間（秒）
  },
  // 打点品質 → 角度幅・球速・精度の変換係数
  contact: {
    idealLateral: 0.6,     // 体の横この距離(m)が適正打点（詰まり判定を緩和）。バック側で使用
    idealLateralFore: 1.3, // フォア側の適正打点距離(m)。フォアは体からもう少し離れて打つため緩める（懐を広げ、より遠い打点まで適正扱いにする）
    ...
    yTolerance: 1.3,     // 前後ズレの許容幅（早い/遅い打点はコース打ち分けに使える駆け引き要素のため、
                         // ペナルティで潰さず意図的に使える程度まで緩める）
 ...
 *
 * y/z（カメラ距離・高さ）とpitch（俯角）は、俯瞰しすぎる「ドローン視点」を弱めて
 * 選手・コートをより近く・低い目線で見せる（臨場感）ため、従来値
 * (y:30.0, z:11.0, pitch:0.62/1.08) から詰めている。fov/horizonYの算出式
 * （applyViewport）や見切れ防止のマージン計算（horizonYのHEAD_Z逆算、
 * player3d.jsの実測フラスタム拡張）はCAM.y/z/pitchを毎回参照して再計算する
 * 構造のため、この3値だけを変えても既存の見切れ防止(#64/#65)は壊れない
 * （変更後、AI対AI観戦モードを長時間走らせ、選手ごとの描画矩形が画面上端で
 * 見切れていないことを数値検証で確認済み。詳細はPR本文の確認観点を参照）。 */
export const CAM = {
  y: 28.5,       // 自陣ベースライン(11.885)後方のカメラ距離（旧30.0。詰めて迫力を出す）
  z: 10.0,       // カメラ高さ（旧11.0。低くして見下ろし感を弱める）
  pitch: 0.58,   // 俯角（横長の基準値。旧0.62。縦長画面ではapplyViewportが上書きする）
...
const CAM_PITCH_BASE = 0.58;  // 横長・正方形に近い画面の俯角（旧0.62。より低い目線に）
const CAM_PITCH_TALL = 1.04;  // 縦長(アスペクト比約2.2以上)の俯角上限（旧1.08）
